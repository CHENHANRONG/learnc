...
- 测试策略是对整个测试项目的整理规划，主要考虑如下内容：
  - 使用的测试标准：这个在实际过程中一般仅作参考，各企业在具体执行时差别较大。具体内容可参考软件产品测试标准
  - 测试风险: 这个是必须考虑的内容，具体规划和执行时可参考浅谈实施软件测试风险分析
  - 测试级别及内容:
  ...
  - 测试入口／出口标准这个标准一般研发内部根据公司标准讨论确定，但一般在实际测试时很难完全遵照执行。
  - 测试用例设计技术
  ...
  - 测试环境包括功能测试环境、性能测试环境、自动化测试环境。如果有完整的业务测试要求还需要端到端的系统／解决方案测试环境。
  - 测试自动化
  ...
  - 测试度量测试部门需要规划从哪些方面去衡量当前产品的质量，对应的衡量要素的具体标准是什么范围，便于后期具体测试时进行评估。
  - 测试管理这个一般有质量管理平台软件来负责，实现对测试全生命周期的管理。常用的有商用软件Quality Center,开源软件JIRA, Redmine等。
  - 测试过程改进一般公司常参考CMMI/TMM/TPI Matrix等过程改进模型，结合公司具体的实际进行改进。
